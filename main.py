...
parser.add_argument('--hp-maxiter', type=int, default=None)
parser.add_argument('--hp-alpha', type=float, default=None)
parser.add_argument('--hn-maxiter', type=int, default=None)
parser.add_argument('--hn-beta', type=float, default=None)
parser.add_argument('--hard-tau', type=float, default=None)
...
# 将命令行参数注入到config_dict中
if temp_args.alpha is not None:
    config_dict['alpha'] = temp_args.alpha
if temp_args.beta is not None:
    config_dict['beta'] = temp_args.beta
if temp_args.hp_maxiter is not None:
    config_dict['hp_maxiter'] = temp_args.hp_maxiter
if temp_args.hp_alpha is not None:
    config_dict['hp_alpha'] = temp_args.hp_alpha
if temp_args.hn_maxiter is not None:
    config_dict['hn_maxiter'] = temp_args.hn_maxiter
if temp_args.hn_beta is not None:
    config_dict['hn_beta'] = temp_args.hn_beta
if temp_args.hard_tau is not None:
    config_dict['hard_tau'] = temp_args.hard_tau
if temp_args.pretrain_weights is not None:
    config_dict['pretrain_weights'] = temp_args.pretrain_weights
if temp_args.weights is not None:
    config_dict['weights'] = temp_args.weights
